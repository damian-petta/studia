...
koniec_t db ?
magazyn db 80 dup (?)
magazyn_W1250 db 80 dup (?)
magzanyn_UTF16 dw 160 dup (?)
title_w1250 db 'Konwerter',0 
title_utf16 dw 'K','o','n','w','e','r','t','e','r', 'U', 'T', 'F',0
...
huge_chars_latin_to_UTF16 dw 0104H, 0106H, 0118H, 0141H, 0143H, 00D3H, 015AH, 0179H,017BH
huge_chars_latin_to_W1250 db 0A5H, 0C6H, 0CAH, 0A3H, 0D1H, 0D3H, 8CH, 8FH, 0AFH
...
_main PROC
	...
	ptl:
		...
		mov magazyn[ebx], dl
		mov magazyn_W1250[ebx], dl
		mov magzanyn_UTF16[ebx*2], dx
		dalej:
	...
	switch_polish_to_upper:
		...
		mov dl, huge_chars_latin_to_W1250[edi]
		mov magazyn_W1250[ebx], dl
		mov dx, huge_chars_latin_to_UTF16[edi*2]
		mov magzanyn_UTF16[ebx*2], dx
		dec eax
		mov ecx, eax
		inc ebx ; inkrementacja indeksu
	...
	koniec:
		...
		push OFFSET title_w1250
		; adres obszaru zawieraj�cego tekst
		push OFFSET magazyn_W1250
		push 0; NULL
		call _MessageBoxA@16

		push 0; sta�a MB_OK
		; adres obszaru zawieraj�cego tytu�
		push OFFSET title_utf16
		; adres obszaru zawieraj�cego tekst
		push OFFSET magzanyn_UTF16
		push 0; NULL
